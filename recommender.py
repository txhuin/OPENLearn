...
def get_courses_by_criteria():
    ...
    #Duration selected 
    #To do: Query database for courses that are more than 20 weeks long
    ...
    # workload_chosen = workload_chosen[0]
    # workload = model.session.query(model.Course)
    get_course = model.session.query(model.Course.course_workload).filter(model.Course.course_workload.like(workload_chosen+'%week')).all()
    print get_course
